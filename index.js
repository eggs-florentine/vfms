...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (interaction.customId === 'ram') {
      ...
      interaction.guild.channels.cache.get('1222773548737892362').send({ embeds: [log] }); // todo: switch to ra-logs channel
    ...
    const author = 'Issued by ' + interaction.member.nickname;

    const notification = new EmbedBuilder()
      .setTitle('Notification of Disciplinary Action')
    ...
    interaction.guild.channels.cache.get('1222773572267675709').send({ embeds: [embed] });
